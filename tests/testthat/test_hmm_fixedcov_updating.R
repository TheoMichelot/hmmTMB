...
mod_tpm0 <<- update(mod_f, "hidden", 1, 2, ~.+x, fit = FALSE, silent = TRUE)
mod_tpm <<- update(mod_tpm0, "hidden", 2, 1, ~.+x, silent = TRUE)
...
test_that("Updating works", {
  ...
  expect_equal(mod_tpm0$hidden()$formulas()[[1]], ~ x) 
  expect_equal(mod_tpm0$hidden()$formulas()[[2]], ~ 1) 
  expect_equal(mod_tpm$hidden()$formulas()[[1]], ~ x) 
  expect_equal(mod_tpm$hidden()$formulas()[[2]], ~ x) 
})
