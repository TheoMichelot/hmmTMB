...
We first create an R object from the `MarkovChain` class, for the hidden state model, indicating that we need a 2-state model.
...
hmm1 <- HMM$new(obs = obs1, hidden = hid1, fixpar = fixpar)
...
The estimates that the detection probability was $p^{(2)}_2 = 0.17$ in years with 2 surveys, and $p^{(3)}_2 = 0.53$ in years with 3 surveys. The colonisation probability over one year was about 0.15, and the extinction probability about 0.19.
...
obs2 <- Observation$new(data = data, dists = dists, 
                       n_states = 2, par = par0)
hmm2 <- HMM$new(obs = obs2, hidden = hid2, fixpar = fixpar)
