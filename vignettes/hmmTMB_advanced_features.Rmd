...
obs_train <- Observation$new(data = energy_train, n_states = 2, 
                             dists = dists, par = par0)
hmm_train <- HMM$new(hidden = hid_train, obs = obs_train)
...
obs_new <- Observation$new(data = energy_new, n_states = 2, 
                           dists = dists, par = par0)
hmm_new <- HMM$new(hidden = hid_new, obs = obs_new, init = hmm_train)
...
hmm <- HMM$new(hidden = hid, obs = obs, fixpar = fixpar)
hmm$fit(silent = TRUE)
...
Similarly, the transition probabilities can be kept fixed or fixed to a common value using `fixpar`. The most common application might be a model where some transitions are prohibited, and therefore should have probability fixed to zero. Consider the 3-state model with transition probability matrix
$$
\Gamma = 
\begin{pmatrix}
...
- `type` refers to the model component that should be changed, either `"hidden"` or `"obs"`.

- `i` and `j` are used to indicate which specific formula should be changed. If `type = "hidden"`, these are the indices of the transition probability for which the formula should be updated. If `type = "obs"`, `i` is the name of the relevant variable, and `j` the name of the relevant parameter.
...
hmm2 <- update(mod = hmm, type = "hidden", i = 1, j = 2, 
               change = ~ . + Oil, fit = FALSE)

# Update formula for Pr(S2 > S1)
hmm2 <- update(mod = hmm2, type = "hidden", i = 2, j = 1, 
               change = ~ . + Oil, fit = FALSE)

# # Update formula for Price mean
# hmm2 <- update(mod = hmm2, type = "obs", i = "Price", j = "mean", 
...
Although it is no substitute for thinking critically about what would be plausible initial parameter values, hmmTMB implements a more automated method to suggest initial values for a given model. This is implemented in the function `suggest_initial()`, which uses the following procedure:
...
obs1 <- Observation$new(data = energy, n_states = 2, 
                       dists = dists, par = par0)
# Create HMM
hmm1 <- HMM$new(hidden = hid1, obs = obs1)
...
hmm2 <- HMM$new(hidden = hid2, obs = obs2, init = hmm1)
hmm2$coeff_fe()
...
# Create HMM with initial values from hmm1
hmm3 <- HMM$new(hidden = hid3, obs = obs3, init = hmm2)
hmm3$coeff_fe()
