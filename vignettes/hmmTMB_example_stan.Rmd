...
\begin{align*}
\log(\mu_1) & \sim N(\log(10), 5^2)\\
\log(\mu_2) & \sim N(\log(30), 5^2)\\
\log(\sigma_1) & \sim N(\log(10), 5^2)\\
\log(\sigma_2) & \sim N(\log(30), 5^2)
\end{align*}
where $\mu_j$ and $\sigma_j$ are the mean and standard deviation of the step length distribution for state $j \in \{1, 2\}$.
...
\begin{align*}
\beta_{ij0} \sim N(-2, 1) \\
\beta_{ij1} \sim N(0, 100) \\
\beta_{ij2} \sim N(0, 100) \\
\end{align*}
