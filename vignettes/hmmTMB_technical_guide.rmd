...
- User vignette: MS-GAM (financial), capture-recapture, bioinformatics, Bayesian example(?)
 ...
 - state probabilities (local decoding) / forward-filtering backward-sampling (RG)
 
 - goodness-of-fit: pseudo-residuals, posterior simulation tests (RG)

 - allow for input of known states (semi-supervised learning) (TM)

 - `stationary = TRUE/FALSE` option in `MarkovChain` class (TM)
 
## Lower priority (difficult?)

 - multivariate distributions (?RG)
   
 - improve parametrisation of mean of circular distributions (avoid numerical issues around -pi and pi) (?TM)
 
 - allow for constraints on parameters (RG)
    ...
    + fixed value, e.g. $\mu_1 = 0$ (using map) (RG)

 - obtain starting values for complex models from simpler nested model (e.g. for smooth effects) (RG)
