...
$$
\begin{aligned}
L(\alpha, \lambda \vert z_1, \dots, z_n) & = \int \exp(l_p) d\beta \\
                                         & = \int [z_1, \dots, z_n \vert \alpha, \beta] [\beta \vert \lambda] d\beta
\end{aligned}
$$
where $[z_1, \dots, z_n \ vert \theta, \beta]$ is the HMM likelihood, and $[\beta \vert \lambda]$ is a multivariate normal pdf with mean zero and block-diagonal precision matrix with blocks $\lambda_i S_i$.
