...
#'
#' @seealso \code{\link{HMM}} for the fitted model that is forecast from,
#' and \code{\link{MarkovChain}} and \code{\link{Observation}} for the
#' hidden state and observation components it is built from.
#'
#' @export
Forecast <- R6Class(
  classname = "Forecast",
  ...
  public = list(
    ...
    #' Algorithm overview
    #' \enumerate{
    #'   \item Validate the arguments.
    #'   \item Build or accept the future design matrix (\code{forecast_data}).
    #'   \item Obtain time-varying transition matrices (\code{tpm_forecast}) and
    #'         observation parameters (\code{obs_par_forecast}) via \code{HMM$predict()}.
    #'   \item Forward-propagate the hidden-state distribution
    #'         (\code{hidden_state_forecast}).
    #'   \item Marginalise over states to obtain unconditional pdfs on the
    #'         evaluation grid (\code{forecast_dists}).
    #' }
    #' 
    #' @param hmm A fitted HMM object.
    #' @param n Forecast horizon: a whole number greater than zero. Ignored if
    #'        forecast_data is provided.
    ...
    #'        For any observation variable without an entry, a default grid of 100
    #'        equally spaced points is used, spanning the range of the training data
    #'        for that variable with each endpoint expanded outward by 10\% of its
    #'        magnitude. A warning is issued whenever this default is applied, as
    #'        the resulting grid may not suit the distribution's support.
    #' @param starting_state_distribution Distribution over states at the first
    #'        forecast step. Either a numeric vector of length nstates() that is
    #'        non-negative and sums to 1, or one of "last" (state probabilities at
    #'        the final training row, propagated one step) or "stationary" (the
    #'        model's stationary distribution).
    #' 
    #' @return A \code{Forecast} object. Results are read back with the accessor
    #' methods below, each invoked with \code{()}:
    #' \describe{
    #'   \item{hidden_state_forecast}{Matrix of nstates rows by n forecast steps.}
    ...
    #'   \item{forecast_data}{The data.frame of future covariates used for prediction.}
    #' }
    #' The evaluation grid can be changed after construction with
    #' \code{update_eval_range()}, which recomputes \code{forecast_dists}.
    #' 
    #' @examples
    #' \donttest{
    #' data(faithful)
    ...
    #' )
    #' hmm$fit(silent = TRUE)
    #' # Create forecast object. Setting the evaluation grid explicitly is
    #' # recommended; omitting it falls back to a default grid and warns.
    #' forecast <- Forecast$new(
    #'   hmm = hmm,
    #'   forecast_data = forecast_df,
    #'   preset_eval_range = list(eruptions = seq(1, 6, length.out = 100)),
    #'   starting_state_distribution = "last"
    #' )
    #'
    #' # Unconditional predictive densities: grid points x forecast steps
    #' dim(forecast$forecast_dists()$eruptions)
    #' }
        ...
        if (is.null(eval_range[[obs_var]])) {
          warning(sprintf(
            paste0("Using a default grid of 100 points for observation variable '%s', ",
                   "spanning the training range with each endpoint expanded by 10%% of ",
                   "its magnitude. Setting eval_range explicitly is recommended."),
            obs_var
          ), call. = FALSE)
        }
