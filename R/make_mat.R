...
make_mat_obs = function(formulas, data, new_data = NULL) {
  ...
  # Loop over variables
  for(i_var in seq_along(formulas)) {
    var_forms <- formulas[[i_var]]
    var_name <- names(formulas)[i_var]
    
    # Loop over parameters
    for(i_par in seq_along(var_forms)) {
      par_forms <- var_forms[[i_par]]
      par_name <- names(var_forms)[i_par]
      
      # Loop over states
      for(i_state in seq_along(par_forms)) {
        form <- par_forms[[i_state]]
        ...
        subnames_fe <- paste0(var_name, ".", par_name, ".", i_state, ".",
                              term_names[1:gam_setup$nsdf])
        names_fe <- c(names_fe, subnames_fe)
        
        # Random effects design matrix
        X_list_re[[k]] <- Xmat[, -(1:gam_setup$nsdf), drop = FALSE]
        if(ncol(X_list_re[[k]]) > 0) {
          subnames_re <- paste0(var_name, ".", par_name, ".", i_state, ".",
                                term_names[-(1:gam_setup$nsdf)])
...
make_mat_hid = function(formulas, data, new_data = NULL) {
  ...
  for(i in seq_along(formulas)) {
    ...
    # Fixed effects design matrix
    X_list_fe[[i]] <- Xmat[, 1:gam_setup$nsdf, drop = FALSE]
    subnames_fe <- paste0(name, ".", term_names[1:gam_setup$nsdf])
    names_fe <- c(names_fe, subnames_fe)
    
    # Random effects design matrix
    X_list_re[[i]] <- Xmat[, -(1:gam_setup$nsdf), drop = FALSE]
    if(ncol(X_list_re[[i]]) > 0) {
      subnames_re <- paste0(name, ".", term_names[-(1:gam_setup$nsdf)])
      names_re <- c(names_re, subnames_re)                    
    }
    
    # Smoothing matrix
    S_list[[i]] <- bdiag_check(gam_setup$S)
